...
describe("<Header />", () => {
  ...
  describe("User query", () => {
    ...
    test("it should render without error", async () => {
      ...
      expect(
        screen.getByTestId("user-avatar-circular-progress")
      ).toBeInTheDocument();
      ...
      expect(screen.getByAltText("avatar-user")).toHaveAttribute(
        "src",
  ...
  describe("Selection of a project", () => {
    test("when there is no error, we should be able to select a project", async () => {
      ...
      (debounce as any).mockImplementation(
        jest.fn((fn: any) => {
      ...
      await waitFor(() =>
        screen.getByLabelText(
          "Choose project First Project, id 1, path /first/project"
        )
      );
      ...
      await waitFor(() =>
        screen.getByLabelText(
          "Choose project Fifth Project, id 5, path /fifth/project"
        )
      );
      ...
      fireEvent.click(
        screen.getByLabelText(
          "Choose project Third Project, id 3, path /third/project"
        )
      );
      ...
      fireEvent.click(
        screen.getByLabelText("Confirm selection of project id 3")
      );
      ...
      fireEvent.click(
        screen.getByLabelText("Close dialog, stop selection of project")
      );
    });
  });
});
