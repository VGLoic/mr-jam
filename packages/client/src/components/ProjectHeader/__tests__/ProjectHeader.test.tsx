...
describe("<ProjectHeader />", () => {
  it("should render with error", async () => {
    ...
    await waitFor(() =>
      screen.getByLabelText("Project with id projectIdTest has not been found")
    );
  ...
  it("should render without error when there are less than 6 users", async () => {
    ...
    expect(
      screen.getByLabelText("Project overview is loading")
    ).toBeInTheDocument();

    expect(asFragment()).toMatchSnapshot("Project Header loading");

    await waitFor(() =>
      screen.getByLabelText("Project name: THE TEST PROJECT")
    );
  ...
  it("should render without error when there are more than 5 users", async () => {
    ...
    expect(asFragment()).toMatchSnapshot(
      "Project header with user dialog open"
    );
