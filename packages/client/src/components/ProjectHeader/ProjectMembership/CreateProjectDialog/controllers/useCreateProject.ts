...
// Hooks
import { useTransaction, TransactionState } from "contexts/ethers/useTransaction";
...
export const useCreateProject = ({
    ...
    const adminUserInputContext = useUserInputs({ defaultUser: user, defaultAddress: selectedAddress });
    const additionalUserInputContext = useUserInputs({ resetParameter: projectName });

    const [additionalUsers, setAdditionalUsers] = useState<Map<string, WrappedUser>>(new Map());
    ...
    const [sendTransaction, transactionState] = useTransaction(
        {
            ...
            args: [
                ...
                Array.from(additionalUsers.values()).map((wrappedUser) => wrappedUser.address),
                Array.from(additionalUsers.values()).map((wrappedUser) => wrappedUser.user.id)
            ]
        }
    );
    ...
    const isConfirmDisabled = adminUserInputContext.isConfirmDisabled ||
