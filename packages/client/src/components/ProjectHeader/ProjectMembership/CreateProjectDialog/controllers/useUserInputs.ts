...
const useUserInputs = ({ defaultUser, defaultAddress, resetParameter }: UseUserInputsArgs = {}): UseUserInputs => {
    const hasMountedRef = useRef<boolean>(false);
    const [userValue, setUserValue] = useState<User | null>(defaultUser || null);
    const handleUserChange = (event: any, value: User | null) => setUserValue(value);

    const [addressValue, setAddressValue] = useState<string>(defaultAddress || "");
    const handleAddressChange = ({ target: { value }}: ChangeEvent<HTMLInputElement>): void => setAddressValue(value);
